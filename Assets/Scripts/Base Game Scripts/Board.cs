...
public class Board : MonoBehaviour
{
            ...
            for (int j = 0; j < height; j++)
            {
                if (!blankSpaces[i, j])
                {
                    ...
                    while (MatchedAt(i, j, dots[dotToUse]) && maxIterations < 100)
                    {
                        ...
                        maxIterations++;
                        Debug.Log("Max Iteration: " + maxIterations);
                    }
                    maxIterations = 0;
    ...
    {
        //Make a copy of a current matches
        List<GameObject> matchCopy = findMatches.currentMatches as List<GameObject>;

        //Cycle through all of match Copy and decide if a bomb needs to be
    ...
    private void CheckToMakeBombs()
    {
        ...
        if (findMatches.currentMatches.Count > 3)
        {
            ...
            {
                //Make a color bomb
                Debug.Log("Color bomb is create");
            ...
            else if(typeOfMatch == 2)
            {
                ...
                Debug.Log("Adjacent bomb is create");
                //Is the current dot matched?
                if (currentDot != null)
                {
    ...
    private void DestroyMatchesAt(int column, int row)
    {
        if (allDots[column, row].GetComponent<Dot>().isMatched)
        {
            ...
            //Create the partical
            GameObject partical = Instantiate(destroyEffect, allDots[column, row].transform.position, Quaternion.identity);
            Destroy(partical, .5f);
    ...
    {
        yield return new WaitForSeconds(refillDelay / 2f);
        Debug.Log("Pieces refil");
        RefillBoard();
        ...
        for (int i = 0; i < width; i++)
        {
            for (int j = 0; j < height; j++)
            {
                ...
                if (!blankSpaces[i, j])
                {
                    ...
                    int maxIterations = 0;
                    while (MatchedAt(i, j, newBoard[pieceToUse]) && maxIterations < 100)
                    {
